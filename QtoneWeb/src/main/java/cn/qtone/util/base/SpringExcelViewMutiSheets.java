 ...
 *
 */
public class SpringExcelViewMutiSheets extends AbstractExcelView {
 ...
 * @param fileName
 */
	public SpringExcelViewMutiSheets(Map<String, String>[] ExcleFieldToBeanFiledMap,
	...
	protected void buildExcelDocument(Map arg0, HSSFWorkbook wb,
		...
		for(int i=0;i<sheetName.length;i++)
		{
			...
			if (needTitle) {
				...
				ExcelUtils.setValue(s, count, 0, title[i]);
			...
			for (int j = 0; j < template[i].length; j++) {
				...
				templateCell.setCellValue(template[i][j]);
			}
			count++;
			
			if(this.beanList[i]!=null&&this.beanList[i].size()>0){
				if(this.beanList[i].get(0) instanceof Map)
					fillRowValueByMapList(i,s,count);
				else
				    fillRowValueByBeanList(i,s,count);
			} 
		}
	}
	 ...
	 * @throws Exception
	 */
	void fillRowValueByMapList(int index,HSSFSheet s,int count) throws Exception{
		
		for (int i = 0; i < beanList[index].size(); i++) {
			
			HSSFRow row = s.createRow(i + count);
			Map<String,Object> mapValues =(Map<String,Object>)beanList[index].get(i);
			 
			 for(String keyName:mapValues.keySet()){
				 Object tempStr = mapValues.get(keyName);
				 tempStr=this.convertValue(tempStr,keyName);
				 for (short k = 0; k < template[index].length; k++) {
					 	//System.out.println("template["+index+"]["+k+"]="+template[index][k]);
						if (template[index][k]!=null&&ExcleFieldToBeanFiledMap[index].get(template[index][k])!=null&&ExcleFieldToBeanFiledMap[index].get(template[index][k]).equals(keyName)) {
	 ...
	 * @throws Exception
	 */
	void fillRowValueByBeanList(int index,HSSFSheet s,int count) throws Exception{

		for (int i = 0; i < beanList[index].size(); i++) {

			HSSFRow row = s.createRow(i + count);

			Object object = beanList[index].get(i);
			//获取实体对象属性名称
			...
			for (int j = 0; j < fields.length; j++) {
				...
				for (short k = 0; k < template[index].length; k++) {
					if (ExcleFieldToBeanFiledMap[index].get(template[index][k]).equals(fields[j].getName())) {
						HSSFCell Cell = row.createCell(k);
						if(tempStr instanceof Long){
							Cell.setCellType(Cell.CELL_TYPE_NUMERIC);
							Cell.setCellValue((Long)tempStr);
